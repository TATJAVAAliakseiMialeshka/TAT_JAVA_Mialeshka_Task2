...
-- Dumping data for table library.books: ~21 rows (approximately)
DELETE FROM `books`;
...
INSERT INTO `books` (`b_id`, `b_name`, `b_year`, `b_description`, `b_quantity`, `b_is_available`) VALUES
	...
	(6, 'The Hobbit', 1937, 'The Hobbit is set in a time "between the Dawn of Færie and the Dominion of Men", and follows the quest of home-loving hobbit Bilbo Baggins to win a share of the treasure guarded by Smaug the dragon. Bilbo\'s journey takes him from light-hearted, rural surroundings into more sinister territory.', 1, 'Y'),
	...
	(8, 'Dream of the Red Chamber', 1754, 'The novel provides a detailed, episodic record of life in the two branches of the wealthy, aristocratic Jia clan—the Rongguo House and the Ningguo House —who reside in two large, adjacent family compounds in the capital. Their ancestors were made Dukes and given imperial titles, and as the novel begins the two houses are among the most illustrious families in the city. One of the clan’s offspring is made a Royal Consort, and a lush landscaped garden is built to receive her visit. The novel describes the Jias’ wealth and influence in great naturalistic detail, and charts the Jias’ fall from the height of their prestige, following some thirty main characters and over four hundred minor ones. Eventually the Jia clan falls into disfavor with the Emperor, and their mansions are raided and confiscated.', 3, 'Y'),
	...
	(28, 'bookname', 1111, 'bookdescr', 99, 'Y');
...
-- Dumping data for table library.books_has_authors: ~22 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `books_has_genres` (
  ...
  CONSTRAINT `fk_books_has_genres_books1` FOREIGN KEY (`b_id`) REFERENCES `books` (`b_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_books_has_genres_genres1` FOREIGN KEY (`g_id`) REFERENCES `genres` (`g_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `users` (`u_id`, `u_name`, `u_password`, `u_status`) VALUES
	(1, 'first_superadmin', 'first', 'A'),
	(2, 'second_admin', 'second', 'A'),
	(3, 'third_admin', 'third', 'A'),
	(4, 'fourth', 'fourth', 'R'),
	(5, 'fifth', 'fifth', 'R'),
	(6, 'sixth', 'sixth', 'R'),
	(7, 'seventh', 'seventh', 'R'),
...
CREATE TABLE IF NOT EXISTS `users_has_role` (
  ...
  CONSTRAINT `fk_users_has_role_role1` FOREIGN KEY (`r_id`) REFERENCES `role` (`r_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_users_has_role_users1` FOREIGN KEY (`u_id`) REFERENCES `users` (`u_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
